...
function TAmqpTransport(client, _options,callback) {
  ...
  this.options = {
    ...
    blocking        : (typeof options.messageBlocking !== 'undefined') ? options.messageBlocking : true,
    msg_timeout     : options.messageTtl || 10,
    log_messages    : (typeof options.logMessages !== 'undefined') ? options.logMessages         : false,
    tempQueuePrefix : options.tempQueuePrefix || 'lycam-amq'
  };
...
TAmqpTransport.prototype.flushQueue = function (msg) {
  ...
  return new Promise(function (resolve, reject) {
    var tempQueueName = self.options.tempQueuePrefix + '.gen-' + uuid.v1();

    self.ch.assertQueue(tempQueueName, {
          exclusive: true,
          autoDelete: true,
        })
      .then(function (_replyQueue) {
          ...
          var blocking = self.options.blocking;
          var msg_timeout = self.options.msg_timeout;
          var log_messages = self.options.log_messages;
          ...
          // If we do not expect a response from downstream clients (Thrift IDL service is oneway)
          if (headers.response_required === false) {
            return self.ch.sendToQueue(self.options.queueName,  msg, options);
          }

          // If we expect to use Thrift IDL as an RPC, and expect this to be a transaction, we wait for a response from downstream.
