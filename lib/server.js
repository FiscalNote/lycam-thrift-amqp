...
function AMQPServer(processor_factory, handler, _options) {
...
// Create a Handler object that will pass along the specified
// RMQ properties when invoked as the first argument.
function createPropertiesHandler(baseHandler, properties) {
  var propertiesHandler = {};
  for (var method in baseHandler) {
    propertiesHandler[method] = function() {
      baseHandler[method].apply(baseHandler, [properties].concat(Array.prototype.slice.call(arguments)));
    }
  }
...
AMQPServer.prototype.onReceiveData = function (ch, data) {
          ...
          var processor = new self.processor_factory(createPropertiesHandler(self.handler, data.properties));
 ...
 * @param {object} processor_factory - A service class or processor function.
 * @param {ServerOptions} options - Optional additional server configuration.
 * @returns {object} - The Apache Thrift Multiplex Server.
 */
exports.createServer = function (processor_factory, handler, options) {
  if (processor_factory.Processor) {
    processor_factory = processor_factory.Processor;
  }

  return new AMQPServer(processor_factory, handler, options);
};
